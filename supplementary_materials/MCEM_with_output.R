...
buildMCEM <- function(model, latentNodes, burnIn = 500 , mcmcControl = list(adaptInterval = 100),
                      ...
                      gamma = 0.05, C = 0.001, numReps = 300, verbose = TRUE,theta0) {
    ...
    while(endCrit > C){
      ...
      while(acceptCrit == 0){
        ...
        if((diff - zAlpha*ase)<0){ #swamped by mc error
          ...
          tempDF <- data.frame(iteration = itNum, params = t(output), convergence = endCrit, mcmcIter = m)
          ## try tacking on Sys.time()
          if(itNum == 1){
